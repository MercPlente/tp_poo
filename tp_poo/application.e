...
feature {NONE} -- Initialization
	...
	set_sound(a_window:GAME_WINDOW)
		...
		local
			l_sound,l_music_intro,l_music_loop,l_second_music_loop:AUDIO_SOUND_FILE
			sound_source,music_source,second_music_source:AUDIO_SOURCE	-- You need one source for each sound you want to be playing at the same time.
		do
			create l_sound.make ("ding.ogg")			-- This sound will be played when the user press the space bar.
			create l_music_intro.make ("intro.ogg")		-- This sound will be played once at the begining of the music
			create l_music_loop.make ("tristram.ogg")		-- This sound will be loop until the application stop.
			create l_second_music_loop.make ("cath.ogg")											-- The library can use every sound file format that the libsndfile library can use (see: http://www.mega-nerd.com/libsndfile)

			if l_sound.is_openable and l_music_intro.is_openable and l_music_loop.is_openable and l_second_music_loop.is_openable then
				l_sound.open
				l_music_intro.open
				l_music_loop.open
				l_second_music_loop.open
				if l_sound.is_open and l_music_intro.is_open and l_music_loop.is_open and l_second_music_loop.is_open  then
	...
	on_key_down_sound(a_timestamp: NATURAL_32; a_key_state: GAME_KEY_STATE; a_sound:AUDIO_SOUND; a_sound_source:AUDIO_SOURCE;a_second_music_source:AUDIO_SOURCE;a_second_music_loop:AUDIO_SOUND;a_music_source:AUDIO_SOURCE;a_music_loop:AUDIO_SOUND)
		...
		do
			...
			if a_key_state.is_a then
				a_music_source.stop
				a_second_music_source.queue_sound_infinite_loop (a_second_music_loop)
				a_second_music_source.play
			end
